...
int main() {
  ...
  std::cout << green << "\t//MY MAIN TESTS//\n\n" << reset
			<< blue << "Creating AAnimal object" << reset
			<< std::endl;
//  AAnimal *animal = new AAnimal();
//  std::cout << red << "AAnimal type: " << animal->getType() << reset << std::endl;
//  animal->makeSound();
//  delete animal;

  std::cout << blue << "\nCreating AAnimal object with Cat type" << reset << std::endl;
  AAnimal *animal = new Cat();
  std::cout << red << "AAnimal type: " << animal->getType() << reset << std::endl;
  animal->makeSound();
  delete animal;

  std::cout << blue << "\nCreating AAnimal object with Dog type" << reset << std::endl;
  animal = new Dog();
  std::cout << red << "AAnimal type: " << animal->getType() << reset << std::endl;
  animal->makeSound();
  delete animal;

  std::cout << blue << "\nCreating AAnimal array" << reset << std::endl;
  AAnimal *animals[4];
  for (int i = 0; i < 2; ++i) {
	animals[i] = new Dog();
	std::cout << red << "AAnimal type: " << animals[i]->getType() << reset << std::endl;
	animals[i]->makeSound();
  }
  for (int i = 2; i < 4; ++i) {
	animals[i] = new Cat();
